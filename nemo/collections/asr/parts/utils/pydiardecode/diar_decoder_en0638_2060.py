    ...
    def get_spk_wise_probs(
        ...
        for spk in speaker_list:
            ...
            for tgt_spk in speaker_list:
                ...
                if spk == tgt_spk:
                    print(f"tgt_spk {tgt_spk} next_word [  {next_word}  ] Indiv sentence: {sentence[-40:]}")
            ...
            # if len(spk_trans_dict[tgt_spk].split()) > 10 and last_spk == tgt_spk:
        ...
        # lm_spk_probs = np.log10(lm_spk_logits)
        # lm_spk_probs = softmax(np.power(10, lm_spk_logits))
        ...
        lm_spk_probs = lm_spk_probs/np.sum(lm_spk_probs)
        print(f"For spk: {spk} Individual softmax probs: {lm_spk_probs}")
        return lm_spk_probs
...
class BeamSearchDecoderCTC:
    ...
    def _get_prev_info(self, spk_int: int, target_text: str, prev_trans_dict: Dict[str, str]) -> Tuple[Dict[str, str], str]:
        ...
        if target_text != '' and len(target_text.split()) > 1:
            ...
            prev_trans_dict[spk_int] = ''
            prev_word = ''
        return prev_trans_dict, prev_word
    ...
    def _get_speaker_lm_beams(
        ...
        for spk_trans_dict, next_word, word_part, last_char, frame_list, frames, logit_score in beams:
            ...
            if cache_key not in cached_lm_scores:
                ...
                prev_trans_dict, prev_word = self._get_prev_info(spk_int=spk_int, target_text=target_text, prev_trans_dict=deepcopy(spk_trans_dict))
                ...
                spk_wise_probs = self.spk_lm_decoder.get_spk_wise_probs(next_word=next_word, 
                                                                        last_char=last_char, 
                                                                        spk_trans_dict=spk_trans_dict,
                                                                        max_word=self.word_window,
                                                                        )
                spk_lm_score = np.log(spk_wise_probs[spk_int])
                # spk_lm_score = np.log(np.clip(spk_wise_probs[spk_int], MIN_TOKEN_CLIP_P, 1))
                # spk_lm_score = spk_wise_probs[spk_int]
                print(f"prev_raw_lm_score {prev_raw_lm_score} spk_lm_score: {spk_lm_score}")
                ...
                cached_lm_scores[cache_key] = (raw_lm_score, spk_lm_score, None)
            ...
            print(f"logit_score: {logit_score}, lm_score: {lm_score} beta*lm_score {self._beta_flag * self.beta * lm_score}")
            # logit_score + self.alpha * lm_score,
            # logit_score + lm_score,
            # (1-self.alpha) * logit_score + self.alpha * lm_score,
            new_beams.append(
                (
                    spk_trans_dict,
                    "",
