...
from nemo.collections.asr.data.audio_to_sot_text_lhotse_prompted import PromptedAudioToTextLhotseDataset
...
from nemo.collections.asr.models import MSEncDecMultiTaskModel, SortformerEncLabelModel
...
"""
RTMTASR / MS Canary transcription entrypoint (fork of transcribe_speech.py).

Same behavior as transcribe_speech.py except ``preserve_speaker_tokens`` defaults to True so
``[s0]``, ``[s1]``, ... stay in ``pred_text`` for SOT cpWER and SegLST conversion. Pass
``preserve_speaker_tokens=false`` to match upstream stripping.
...
def _build_sot_dataset(asr_model) -> PromptedAudioToTextLhotseDataset:
    ...
    sot_cfg = {
        ...
        'sample_rate': asr_model.cfg.get('sample_rate', 16000),
        'window_stride': asr_model.cfg.get('preprocessor', {}).get('window_stride', 0.01),
        'subsampling_factor': subsampling_factor,
    }
    return PromptedAudioToTextLhotseDataset(
        tokenizer=asr_model.tokenizer,
        prompt=asr_model.prompt,
        sot_cfg=sot_cfg,
    )
...
def load_rttm_speaker_matrices(sorted_manifest_path: str, asr_model) -> torch.Tensor:
    ...
    PromptedAudioToTextLhotseDataset._fix_speaker_activity.
    ...
    rttm_mats = []
    with open(sorted_manifest_path, 'r', encoding='utf-8') as mf:
        for line_num, line in enumerate(mf):
            item = json.loads(line)
            rttm_path = item.get('rttm_filepath')
            if not rttm_path:
            ...
            rttm_timestamps, _ = extract_frame_info_from_rttm(offset, item['duration'], rttm_lines)
            ...
            text = item.get('text', '')
            if text:
                fake_cut = types.SimpleNamespace(text=text, id=f"line_{line_num}")
                rttm_mat = sot_dataset._fix_speaker_activity(fake_cut, rttm_mat)
...
def load_diar_model_speaker_matrices(
    diar_model_path: str,
    manifest_path: str,
    ...
    """Load a Sortformer diarization model, run inference, return collated predictions.

    Returns:
        Collated (N, T_max, num_speakers) tensor of sigmoid predictions.
    """
    logging.info(f"Loading external diarization model from {diar_model_path}")
    if diar_model_path.endswith(".ckpt"):
        diar_model = SortformerEncLabelModel.load_from_checkpoint(
            checkpoint_path=diar_model_path, map_location=map_location, strict=False,
        )
    elif diar_model_path.endswith(".nemo"):
        diar_model = SortformerEncLabelModel.restore_from(
            restore_path=diar_model_path, map_location=map_location,
        )
    else:
        raise ValueError(f"diar_model_path must end with .ckpt or .nemo, got: {diar_model_path}")

    diar_model._cfg.test_ds.manifest_filepath = manifest_path
    ...
    diar_trainer = pl.Trainer(devices=device_list, accelerator=accelerator)
    diar_model.set_trainer(diar_trainer)
    diar_model = diar_model.eval()

    logging.info(f"Running diarization inference on {manifest_path} ...")
    with torch.inference_mode():
        diar_model.test_batch()

    preds_list = diar_model.preds_total_list
    diar_mats = [p.squeeze(0) for p in preds_list]
    rttms_mask_mats = collate_matrices(diar_mats)

    del diar_model
    torch.cuda.empty_cache()

    return rttms_mask_mats


def calculate_cpwer_from_manifest(
    pred_manifest: str,
    gt_text_attr_name: str = "text",
    pred_text_attr_name: str = "pred_text",
    remove_pnc: bool = True,
) -> float:
    """Compute corpus-level cpWER by comparing pred_text vs ground-truth text in a manifest.

    Reads the output manifest (which has both GT and predicted text), splits each
    by speaker tags, and computes session-level cpWER via Hungarian assignment.
    ...
    with open(pred_manifest, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            hyp_text = item.get(pred_text_attr_name, "")

            spk_ref = split_text_by_speaker_tags(ref_text, remove_pnc=remove_pnc)
            spk_hyp = split_text_by_speaker_tags(hyp_text, remove_pnc=remove_pnc)
...
@dataclass
class TranscriptionConfigRTMTASR:
    """
    Transcription Configuration for audio to text transcription (RTMTASR / MS Canary).
    """

    # Required configs
    model_path: Optional[str] = None  # Path to a .nemo file or .ckpt checkpoint
    diar_model_path: Optional[str] = None  # Path to a .nemo file or .ckpt checkpoint
    ...
    # False: strip [s*] like upstream transcribe_speech.py. True: keep for SOT / cpWER / SegLST.
    preserve_speaker_tokens: bool = True

    # Speaker supervision source for encoder infusion:
    #   "diar" + diar_model_path set  → load external Sortformer diar model, run inference, use its predictions.
    #   "diar" + diar_model_path null → use ASR model's internal diarization head.
    #   "rttm"                        → load ground-truth speaker activity from rttm_filepath in each manifest entry.
    spk_supervision: str = "diar"


@hydra_runner(config_name="TranscriptionConfigRTMTASR", schema=TranscriptionConfigRTMTASR)
def main(cfg: TranscriptionConfigRTMTASR) -> Union[TranscriptionConfigRTMTASR, List[Hypothesis]]:
    """
    ...
    # ---- Speaker supervision: load speaker activity matrices ----
    asr_device = next(asr_model.parameters()).device
    if cfg.spk_supervision == "rttm" and sorted_manifest_path is not None:
        rttms_mask_mats = load_rttm_speaker_matrices(sorted_manifest_path, asr_model)
        asr_model.add_rttms_mask_mats(rttms_mask_mats, device=asr_device)
        logging.info(f"Loaded RTTM speaker matrices (shape {rttms_mask_mats.shape})")

    elif cfg.spk_supervision == "diar" and cfg.diar_model_path is not None:
        manifest_for_diar = sorted_manifest_path or cfg.dataset_manifest
        rttms_mask_mats = load_diar_model_speaker_matrices(
            diar_model_path=cfg.diar_model_path,
            manifest_path=manifest_for_diar,
        ...
        asr_model.add_rttms_mask_mats(rttms_mask_mats, device=asr_device)
        logging.info(f"Diar model speaker matrices (shape {rttms_mask_mats.shape})")
    ...
    if not cfg.preserve_speaker_tokens:
    ...
    if cfg.calculate_cpwer:
        cpwer = calculate_cpwer_from_manifest(
            pred_manifest=output_filename,
            gt_text_attr_name=cfg.gt_text_attr_name,
            pred_text_attr_name=pred_text_attr_name,
    ...
    if cfg.spk_supervision == "rttm":
        spk_method = "RTTM Supervisions (ground-truth speaker activity from rttm_filepath)"
    elif cfg.spk_supervision == "diar" and cfg.diar_model_path is not None:
        spk_method = f"External Diarizer ({cfg.diar_model_path})"
    elif cfg.spk_supervision == "diar":
        spk_method = "Internal Diarizer (model's own diarization head, no external diar_model_path)"
    else:
        spk_method = f"Unknown (spk_supervision={cfg.spk_supervision})"
    logging.info("=" * 70)
    logging.info(f"  Speaker supervision method: {spk_method}")
    logging.info("=" * 70)
