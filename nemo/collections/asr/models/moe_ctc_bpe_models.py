...
"""
CTC BPE model with Mixture-of-Experts (MoE) encoder auxiliary loss support.

Extends EncDecCTCModelBPE to collect and add the MoE load-balancing auxiliary
...
__all__ = ['EncDecMoECTCModelBPE']


class EncDecMoECTCModelBPE(EncDecCTCModelBPE):
    """CTC BPE model with MoE encoder auxiliary loss support.

    Inherits all functionality from :class:`EncDecCTCModelBPE` and overrides
    :meth:`add_auxiliary_losses` to collect the MoE load-balancing loss from the
    encoder during training.

    Use this model class with :class:`MoEConformerEncoder` to train CTC models
