...
class SortformerEncLabelModel(ModelPT, ExportableEncDecModel):
    ...
    def _reset_train_f1_accs(self):
        ...
        self._accuracy_train_toplyr.reset()
        self._accuracy_train_prdmean.reset()
    ...
    def training_step(self, batch: list, batch_idx: int):
        ...
        loss = spk_loss
        self._accuracy_train.update(preds, targets, sequence_lengths)
    ...
    def validation_step(self, batch: list, batch_idx: int, dataloader_idx: int = 0):
        ...
        preds_vad, preds_ovl, targets_vad, targets_ovl = self.compute_aux_f1(preds, targets)
        self._accuracy_valid_vad.update(preds_vad, targets_vad, sequence_lengths)
        self._accuracy_valid_ovl.update(preds_ovl, targets_ovl, sequence_lengths)
        valid_f1_vad = self._accuracy_valid_vad.compute()
        valid_f1_ovl = self._accuracy_valid_ovl.compute()
        loss = spk_loss
        self._accuracy_valid.update(preds, targets, sequence_lengths)
