...
$ python <nemo_root_path>/scripts/dataset_processing/tts/compute_speaker_stats.py \
	...
	--stats_path=<data_root_path>/pitch_stats.json
"""
...
def get_args():
    ...
    parser.add_argument(
        "--stats_path",
        default=Path("speaker_stats.json"),
        type=Path,
        help="Path to output JSON file with speaker statistics.",
    )
...
def main():
    ...
    pitch_dir = os.path.join(sup_data_path, Pitch.name)
    ...
    for entry in tqdm(entries):
        pitch_path = get_sup_data_file_path(entry, base_dir, sup_data_path, Pitch)
    ...
    with open(stats_path, 'w', encoding="utf-8") as stats_f:
        json.dump(speaker_stats, stats_f, indent=4)
