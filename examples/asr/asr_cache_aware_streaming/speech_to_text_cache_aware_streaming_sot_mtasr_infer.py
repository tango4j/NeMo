...
class TranscriptionConfig:
    ...
    # ── Offline comparison ─────────────────────────────────────────────
    compare_vs_offline: bool = False  # Whether to compare the output with offline mode
...
def perform_streaming(
    ...
    """
    Perform cache-aware streaming inference.

    If the model is an MSEncDecRNNTBPEModel with diarization enabled
    (asr_model.diar == True), this function automatically uses
    conformer_stream_step_with_diarization() to fuse streaming diarization
    predictions with the ASR encoder states at every chunk.  This adds
    speaker_infusion_asr to the encoder output, which is critical for
    multi-talker ASR in streaming mode where cache context is limited (~5 s).

    Otherwise, it falls back to the standard conformer_stream_step() path.
    """
    ...
    if compare_vs_offline:
        ...
        # in order to compare the results with the streaming mode.
    ...
    if compare_vs_offline:
        # Calculate and report the differences between the predictions of
        # the model in offline mode vs streaming mode.
        ...
        if pred_out_stream_cat.size() == pred_out_offline_cat.size():
            ...
            logging.info(
                f"The shape of the outputs of the model in streaming mode ({pred_out_stream_cat.size()}) "
                f"is different from offline mode ({pred_out_offline_cat.size()})."
            )
