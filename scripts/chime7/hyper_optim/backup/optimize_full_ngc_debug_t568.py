...
OUTPUT_DIR = "/ws/chime7_outputs/optuna-msdd-gss-asr5-trial568"
...
def diar_config_setup(
    ...
    config.diarizer.vad.parameters.frame_vad_threshold = 0.22 #trial.suggest_float("frame_vad_threshold", 0.23, 0.23, step=0.005)
    config.diarizer.vad.parameters.pad_onset = 0.15  #round(trial.suggest_float("pad_onset", 0.15, 0.15, step=0.01), 2)
    config.diarizer.vad.parameters.pad_offset = 0.5  #round(trial.suggest_float("pad_offset", 0.35, 0.35, step=0.01), 2)
    config.diarizer.vad.parameters.min_duration_on = 0.45 #round(trial.suggest_float("min_duration_on", 0.4, 0.4, step=0.05), 2)
    config.diarizer.vad.parameters.min_duration_off = 0.55 # round(trial.suggest_float("min_duration_off", 0.95, 0.95, step=0.05), 2)

    # MSDD Optimization
    config.diarizer.msdd_model.parameters.sigmoid_threshold = [0.6]
    config.diarizer.msdd_model.parameters.global_average_mix_ratio = 1.0  #trial.suggest_float("global_average_mix_ratio", low=0.75, high=0.75, step=0.05)

    # Clustering Optimization
    config.diarizer.clustering.parameters.max_rp_threshold = 0.05
    config.diarizer.clustering.parameters.sparse_search_volume = 25
    r_value = 0.35  #round(trial.suggest_float("r_value", 1.7, 1.7, step=0.05), 4)
...
def objective_gss_asr(
    ...
    mc_mask_min_db = -100  #trial.suggest_int("mc_mask_min_db", -160, -160, 20)
    mc_postmask_min_db = -15  #trial.suggest_int("mc_postmask_min_db", -12, -12, 3)
    bss_iterations = 5
    dereverb_filter_length = 5
    normalize_db = -35  #trial.suggest_int("normalize_db", -20, -20, 5)
    top_k = 70  

    # New parameters
    dereverb_prediction_delay = 2 # trial.suggest_categorical("dereverb_prediction_delay", choices=[3])
    dereverb_num_iterations = 10 #trial.suggest_categorical("dereverb_num_iterations", choices=[5])
    mc_filter_type = 'pmwf'
