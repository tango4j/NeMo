...
class SquimMetric(torch.nn.Module):
    ...
    def forward(self, channels):
        assert channels.ndim == 3
        assert channels.size(0) == 1

        if self.metric in ['stoi', 'pesq', 'si-sdr']:
            model_output = self.model(channels[0])
            if self.metric == 'stoi':
        ...
        elif self.metric == 'mos':
            num_channels = channels.size(1)
            score = self.model(channels[0], WAVEFORM_NMR.expand([num_channels, -1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metric",
        default='ev',
        choices=['ev', 'si-sdr', 'pesq', 'stoi', 'mos'],
        help="Metric for channel ranking",
    )
    ...
    if args.metric == 'ev':
        ...
        ranker = SquimMetric(sample_rate=recordings[0].sampling_rate, metric=args.metric)
