...
def get_gss_command(gpu_id, diar_config, diar_param, diar_base_dir, output_dir, mc_mask_min_db, mc_postmask_min_db,
    ...
    command = f"MAX_SEGMENT_LENGTH=1000 MAX_BATCH_DURATION=20 BSS_ITERATION={bss_iterations} MC_MASK_MIN_DB={mc_mask_min_db} MC_POSTMASK_MIN_DB={mc_postmask_min_db} DEREVERB_FILTER_LENGTH={dereverb_filter_length} TOP_K={top_k}" \
...
def objective_gss_asr(
    ...
    mc_postmask_min_db = trial.suggest_int("mc_postmask_min_db", -9, -9, 3)
    ...
    normalize_db = trial.suggest_int("normalize_db", -30, -30, 5)
    top_k = trial.suggest_int("top_k", 80, 80, 20)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest_path", help="path to the manifest file", type=str, default="./manifests_dev_taejin")
    parser.add_argument(
        "--config_url",
