...
def _maybe_mount_pe_encoder(perception: nn.Module, pe_encoder_path: str | None) -> bool:
    """Replace ``perception.encoder`` with a ParallelExpertEncoder bundle.

    Training-time SALM (``setup_parallel_expert_encoder``) builds the plain
    ``AudioPerceptionModule`` from ``config.perception`` and then swaps its
    ``encoder`` for a :class:`ParallelExpertEncoder` loaded from
    ``model.pe_encoder_path``. The serialized HF ``config.json`` only records the
    inner ConformerEncoder under ``perception.encoder``, so a checkpoint trained
    with a PE encoder carries nested weight names
    (``perception.encoder.asr_encoder.*`` / ``...diarization_model.*`` /
    ``...asr_norm`` / ``...diar_norm``). Without re-mounting the PE encoder here,
    the loaded module would be a flat ConformerEncoder
    (``perception.encoder.layers.*``) and every weight would fail to initialize.

    Returns ``True`` when a PE encoder was mounted, ``False`` otherwise.
    """
    ...
    # ``load_parallel_expert_encoder_from_nemo`` restores onto CPU (NeMo
    # ``restore_from`` builds real CPU tensors), ignoring vLLM's on-device
    # construction context. The encoder it replaces was created on the worker's
    # GPU, so copy that device/dtype onto the PE encoder; otherwise the whole
    # perception is dragged back to CPU (``_process_audio`` reads
    # ``next(perception.parameters()).device``) and runs into mixed CPU dtype
    # errors. A later ``perception.to(bfloat16)`` in ``_load_perception_weights``
    # keeps the dtype consistent.
    ...
    # The PE encoder consumes un-normalised mels for its Sortformer branch and
    # replays ASR normalisation internally, so disable the outer preprocessor
    # normalisation (mirrors setup_parallel_expert_encoder).
    try:
