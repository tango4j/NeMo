...
def write_rttm2manifest(
    AUDIO_RTTM_MAP: str, 
    manifest_file: str, 
    min_segment_duration: float = 0.03,
    include_uniq_id: bool = False,
    decimals: int = 5, 
...
def segments_manifest_to_subsegments_manifest(
    ...
    with open(segments_manifest_file, 'r') as segments_manifest, open(
        ...
        for segment in segments_manifest.readlines():
            ...
            if get_dict:
                ts_array = start_dur_to_start_end(subsegments)
                if uniq_id in subsegment_dict:
                    # subsegment_dict[uniq_id].extend(ts_list)
                    subsegment_dict[uniq_id] = np.stack(ts_array, axis=0)
                else:
                    subsegment_dict[uniq_id] = ts_array
            else:
    ...
    for part_dict in _subsegments_manifest_dict_list:
        for key in part_dict:
            if key in _subsegments_manifest_dict:
                _subsegments_manifest_dict[key] = np.vstack((part_dict[key], _subsegments_manifest_dict[key]))
            else:
...
def prepare_split_data(
    ...
    if global_rank == 0:
        ...
        return multiscale_timestamp_dict
    ...
    multiscale_timestamps_dict = get_timestamps(multiscale_timestamps_by_scale, multiscale_args_dict)
    save_subsegment_files(multiscale_timestamps_dict, speaker_dir)
...
def save_subsegment_files(multiscale_timestamps_dict, speaker_dir):
    ...
    Args:
        ...
        speaker_dir (str): Directory to save the subsegment files.
    """
    source_session_dict = {}
    for uniq_id, ms_dict_for_source_uniq_id in tqdm(multiscale_timestamps_dict.items(), desc='Splitting subsegment IDs', unit=' IDs', leave=False):
        # The file name convention is <uniq_id>.<speaker1>_<speaker2>_<start time in ms>_<end time in ms>
        source_file_uniq_id = uniq_id.rsplit('.', 1)[0]
        if source_file_uniq_id not in source_session_dict:
            source_session_dict.update({source_file_uniq_id: {uniq_id: ms_dict_for_source_uniq_id}})
        else:
            if uniq_id not in source_session_dict[source_file_uniq_id]:
                source_session_dict[source_file_uniq_id].update({uniq_id: ms_dict_for_source_uniq_id})
            elif uniq_id in source_session_dict[source_file_uniq_id]:
                source_session_dict[source_file_uniq_id][uniq_id].update(ms_dict_for_source_uniq_id)
    
    # Save each source file in uniq id to each pickle file
    for source_file_uniq_id, scale_dict_for_uniq_ids in tqdm(source_session_dict.items(), desc='Saving subsegment files', unit=' files', leave=False):
        with open(os.path.join(speaker_dir, f'{source_file_uniq_id}.pkl'), 'wb') as f:
            pickle.dump(scale_dict_for_uniq_ids, f)
