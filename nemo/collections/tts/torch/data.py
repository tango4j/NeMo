...
class TTSDataset(Dataset):
    ...
    def add_pitch(self, **kwargs):
        ...
        if self.pitch_norm:
            # XOR to validate that neither or both pitch stats are provided
            ...
            if (self.pitch_mean is None) == (pitch_stats_path is None):
                raise ValueError(f"pitch_norm requires exactly 1 of (pitch_mean, pitch_std) or pitch_stats_path. "
        ...
        if pitch is not None:
            ...
            if self.pitch_norm:
                if self.pitch_mean is not None and self.pitch_std is not None:
                    ...
                    speaker_str = str(sample["speaker_id"])
                    if speaker_str in self.pitch_stats:
                        pitch_stats = self.pitch_stats[speaker_str]
                    elif "default" in self.pitch_stats:
                        pitch_stats = self.pitch_stats["default"]
                    else:
                        raise ValueError(f"Could not find pitch stats for speaker {speaker_str}.")
