...
"""
frame_vad_threshold: 0.61, 
global_average_mix_ratio: 0.1, 
global_average_window_count: 110, 
lm_alpha: 0.0, 
lm_beam_size: 7, 
maes_expansion_gamma: 4.3, 
max_rp_threshold: 0.07, 
mc_postmask_min_db: -16, 
min_duration_off: 0.55, 
min_duration_on: 0.2, 
pad_offset: 0.0, 
pad_onset: 0.16, 
r_value: 2.4000000000000004, 
...
def diar_config_setup(
    ...
    config.diarizer.vad.parameters.frame_vad_threshold = 0.61 #trial.suggest_float("frame_vad_threshold", 0.15, 0.7, step=0.02)
    config.diarizer.vad.parameters.pad_onset = 0.0 #round(trial.suggest_float("pad_onset", 0.0, 0.2, step=0.01), 2)
    config.diarizer.vad.parameters.pad_offset = 0.16 #round(trial.suggest_float("pad_offset", 0.0, 0.2, step=0.01), 2)
    config.diarizer.vad.parameters.min_duration_on = 0.2 #round(trial.suggest_float("min_duration_on", 0.2, 0.4, step=0.05), 2)
    ...
    config.diarizer.msdd_model.parameters.global_average_mix_ratio = 0.1 #trial.suggest_float("global_average_mix_ratio", low=0.1, high=0.95, step=0.05)
    config.diarizer.msdd_model.parameters.global_average_window_count = 110 #trial.suggest_int("global_average_window_count", low=10, high=500, step=20)
    ...
    r_value = 2.4  #round(trial.suggest_float("r_value", 0.5, 2.5, step=0.05), 4)
...
def objective_gss_asr(
    ...
    lm_beam_size = 7 # trial.suggest_int(name='lm_beam_size', low=6, high=8, step=1)
    maes_num_steps = 5
    maes_alpha = 3
    maes_gamma = 4.3 #trial.suggest_float(name="maes_expansion_gamma", low=0.3, high=5.3, step=0.5)
    maes_beta = 5
