...
OUTPUT_DIR = "/ws/chime7_outputs/optuna-msdd-gss-asr5-trial429"
...
def diar_config_setup(
    ...
    config.diarizer.vad.parameters.frame_vad_threshold = 0.24 #trial.suggest_float("frame_vad_threshold", 0.23, 0.23, step=0.005)
    ...
    config.diarizer.vad.parameters.min_duration_off = 0.75 # round(trial.suggest_float("min_duration_off", 0.95, 0.95, step=0.05), 2)

    # MSDD Optimization
    config.diarizer.msdd_model.parameters.sigmoid_threshold = [0.75]
    config.diarizer.msdd_model.parameters.global_average_mix_ratio = 0.95  #trial.suggest_float("global_average_mix_ratio", low=0.75, high=0.75, step=0.05)
    ...
    r_value = 0.4  #round(trial.suggest_float("r_value", 1.7, 1.7, step=0.05), 4)
...
def objective_gss_asr(
    ...
    mc_mask_min_db = -120  #trial.suggest_int("mc_mask_min_db", -160, -160, 20)
